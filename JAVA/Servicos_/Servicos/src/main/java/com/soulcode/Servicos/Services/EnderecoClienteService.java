...
public class EnderecoClienteService {
    ...
    @Cacheable("enderecosClienteCache")
    public List<EnderecoCliente> mostrarEnderecosCliente(){
        return enderecoClienteRepository.findAll();
    }
    @Cacheable (value = "enderecosClienteCache", key = "#idCliente")
    public EnderecoCliente mostrarEnderecoPeloId(Integer idCliente){
        Optional<EnderecoCliente> endereco = enderecoClienteRepository.findById(idCliente);
        return endereco.orElseThrow();
    }
